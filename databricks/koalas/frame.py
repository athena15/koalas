...
defaultdict(<class 'list'>, {'col..., 'col...})]
    ...
    def drop_duplicates(self, subset: Optional[Union[str, List[str]]] = None,
                        keep: Optional[Union[str, bool]] = 'first',
                        inplace: Optional[bool] = False) -> 'DataFrame':
        """
        ...
        """
        if keep is not 'first':
            raise NotImplementedError('Only keep=\'first\' is currently implemented.')
        sdf = self._sdf.drop_duplicates(subset)
        if inplace:
            self._sdf = sdf
        return DataFrame(sdf, self._metadata.copy())
